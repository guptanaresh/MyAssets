...
@synthesize name;
@synthesize parentID, fieldname1, fieldname2, fieldtype1, fieldtype2, serverpk;
...
+ (NSInteger)insertNewCategoryIntoDatabase:(sqlite3 *)database {
    ...
    if (insert_statement == nil) {
        static char *sql = "INSERT INTO category (name) VALUES('')";
...
+ (NSMutableArray *)retrieveAllCategories:(sqlite3 *)database {
    ...
    if (retrieve_all_statement == nil) {
		...
		static char *sql = "SELECT * FROM category ORDER BY `name`";
	...
	while (sqlite3_step(retrieve_all_statement) == SQLITE_ROW) {
		...
		Category *cat = [[Category alloc] initWithPrimaryKey:pk database:database];
...
- (BOOL)deleteFromDatabase {
	...
	if (delete_statement == nil) {
		const char *sql = "DELETE FROM category WHERE pk=?";
...
- (void)fromDB {
    ...
    if (hydrate_statement == nil) {
        const char *sql = "SELECT * FROM category WHERE pk=?";
    ...
    if (success == SQLITE_ROW) {
        ...
        self.fieldname1 = (str) ? [NSString stringWithUTF8String:str] : @"";
        str = (char *)sqlite3_column_text(hydrate_statement, 4);
        self.fieldtype1 = (str) ? [NSString stringWithUTF8String:str] : @"";
        str = (char *)sqlite3_column_text(hydrate_statement, 5);
        self.fieldname2 = (str) ? [NSString stringWithUTF8String:str] : @"";
        str = (char *)sqlite3_column_text(hydrate_statement, 6);
        self.fieldtype2 = (str) ? [NSString stringWithUTF8String:str] : @"";
        self.serverpk = (NSInteger)sqlite3_column_int(hydrate_statement, 7);
	} else {
...
- (void)toDB {
	...
	if (dehydrate_statement == nil) {
		const char *sql = "UPDATE category SET parentID=?, name=? , fieldname1=? , fieldtype1=? , fieldname2=? , fieldtype2=?, serverpk=? WHERE pk=?";
	...
	sqlite3_bind_text(dehydrate_statement, 3, [fieldname1 UTF8String], -1, SQLITE_TRANSIENT);
	sqlite3_bind_text(dehydrate_statement, 4, [fieldtype1 UTF8String], -1, SQLITE_TRANSIENT);
	sqlite3_bind_text(dehydrate_statement, 5, [fieldname2 UTF8String], -1, SQLITE_TRANSIENT);
	sqlite3_bind_text(dehydrate_statement, 6, [fieldtype2 UTF8String], -1, SQLITE_TRANSIENT);
	sqlite3_bind_int(dehydrate_statement, 7, serverpk);
	sqlite3_bind_int(dehydrate_statement, 8, pk);
...
- (BOOL)isEqual:(id)anObject{
	Category *crs=(Category *)anObject;
...
+(BOOL)uploadAllCategories:(sqlite3 *)database forUser:(User *)aUser
{
	if([[Reachability sharedReachability] internetConnectionStatus] == NotReachable){
		...
		NSMutableArray *catList = [Category retrieveAllCategories:database];
		NSURL *url = [NSURL URLWithString:@"http://www.jajsoftware.com/myassets/categories_upload.php"]; 
		NSMutableURLRequest *request = [[[NSMutableURLRequest alloc] initWithURL:url] autorelease];
		[request setHTTPMethod:@"POST"];
		
		NSMutableString *post = [NSMutableString stringWithFormat:@"userID=%i", aUser.serverpk];
		NSInteger count=[catList count];
		for(int i=0; i <count; i++){
			Category	*cat = [catList objectAtIndex:i];
			[post appendFormat:@"&pk[%i]=%i",i,cat.pk];
			[post appendFormat:@"&parentID[%i]=%i",i,cat.parentID];
			[post appendFormat:@"&name[%i]=%@",i,cat.name];
			[post appendFormat:@"&fieldname1[%i]=%@",i,cat.fieldname1];
			[post appendFormat:@"&fieldtype1[%i]=%@",i,cat.fieldtype1];
			[post appendFormat:@"&fieldname2[%i]=%@",i,cat.fieldname2];
			[post appendFormat:@"&fieldtype2[%i]=%@",i,cat.fieldtype2];
		}
